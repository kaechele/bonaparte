...
function p_efire.dissector(buffer, pinfo, root)
  ...
  if msg_type == 0xaa then
    ...
    if command == 0xb1 then
      ...
      print(state)
      param_tree:add(f_light_state, buffer(4, 3), (t_light_states[state] and t_light_states[state])):set_generated()
    end
    ...
    if command == 0xc1 then
      param_tree
        :add(
          f_light_color,
          buffer(4, 3),
          "R: " .. parameters:get_index(0) .. " G: " .. parameters:get_index(1) .. " B: " .. parameters:get_index(2)
        )
        :set_generated()
    end
